...
def add_agent_to_path(agent_app_path):
    ...
    if p not in sys.path: sys.path.insert(0, p); added = True
    else: added = False
    yield
...
def pytest_sessionfinish(session, exitstatus):
    ...
    for _, label, key in SECTIONS:
        tests = _results[key]; total = len(tests)
        passed = sum(1 for t in tests if t["outcome"] == "passed")
        failed = sum(1 for t in tests if t["outcome"] == "failed")
        s: dict[str, Any] = {"name": label, "marker": key, "total": total, "passed": passed, "failed": failed, "skipped": total - passed - failed, "score": round(passed / total * 100, 2) if total else 0.0, "tests": tests}
        if key == "agent_tests":
