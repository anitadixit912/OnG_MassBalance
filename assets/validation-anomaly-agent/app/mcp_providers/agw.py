...
def _build_mock_tools():
    ...
    for _, server in mock_data.get("servers", {}).items():
        for tool_name, tool_def in server.get("tools", {}).items():
            ...
            fd = {}
            for fn, fi in props.items():
                t = int if fi.get("type") == "integer" else float if fi.get("type") == "number" else bool if fi.get("type") == "boolean" else str
                fd[fn] = (t, Field(description=fi.get("description", ""))) if fn in required else (t, Field(default=None, description=fi.get("description", "")))
...
async def get_mcp_tools():
    ...
    agw = create_client()
    mcp = await agw.list_mcp_tools(user_token=_get_user_token)
    if not mcp: return []
    def _mk(t):
