...
_PATTERNS = [(re.compile(p, re.IGNORECASE), n) for p, n in [(r"(ignore|disregard)\s+(all\s+)?(previous|prior|above|your)?\s*(instructions|rules|guidelines)", "instruction_override"), (r"you\s+are\s+now\s+(a|an)\s+", "role_manipulation"), (r"(reveal|show|output|print)\s+(your\s+)?(system\s+prompt|instructions)", "prompt_disclosure"), (r"<\|im_start\|>", "delimiter_escape"), (r"\[SYSTEM\]", "delimiter_escape")]]
@dataclass
class ScanResult:
...
def scan_content(content):
    ...
    for p, n in _PATTERNS:
        if p.search(content): return ScanResult(True, n, content, f"[CONTENT BLOCKED: ({n})]")
