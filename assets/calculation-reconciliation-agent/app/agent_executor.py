        ...
        try:
            async for item in self.agent.stream(query, task.context_id, tools=tools):
                if item["require_user_input"]:
                    ...
                    await updater.add_artifact([Part(root=TextPart(text=item["content"]))], name="calculation_result"); await updater.update_status(TaskState.completed, new_agent_text_message(item["content"], task.context_id, task.id), final=True); break
