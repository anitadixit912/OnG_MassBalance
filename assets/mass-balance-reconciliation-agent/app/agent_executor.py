        ...
        try:
            async for item in self.agent.stream(query, task.context_id, tools=tools):
                ...
                if require_user_input:
                    await updater.update_status(TaskState.input_required, new_agent_text_message(content, task.context_id, task.id), final=True)
                    break
                    ...
                    await updater.update_status(TaskState.working, new_agent_text_message(content, task.context_id, task.id))
        except Exception as e:
